...
class Another_P_at_K_Loss(nn.Cell):
	...
	def forward(self, embedding, label):
		...
		eyes_ = ops.eye(batch_size, batch_size, mindspore.int32)
		y_=label.expand(batch_size, batch_size).eq(label.expand(batch_size, batch_size).t()).float()
		score = ops.masked_select(score, 1-eyes_).view(batch_size,-1)
		y_ = ops.masked_select(y_, 1-eyes_).view(batch_size,-1)
