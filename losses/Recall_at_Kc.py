...
class Another_P_at_K_Loss(nn.Cell):
	...
	def forward(self, f, embedding, label):
		embd_size = embedding.size(1)#64
		batch_size = embedding.size(0)#64
		relabel = ops.argmax(label, axis=1)
		relabel,_ = ops.sort(relabel)
		# print('label.shape',relabel)

		sample_list=[]
		n_pos_temp=[]
		categories=-1 
		for i in range(batch_size):#将所有的类有多少个记录下来
			if relabel[i] not in sample_list:
				sample_list.append(relabel[i])
				categories+=1
				n_pos_temp.append(1)
			else:
				n_pos_temp[categories]+=1
		n_pos=Tensor(n_pos_temp)
		# Compute similarity matrix
		score = ops.matmul(embedding, embedding.T)
		s, label_set_index = score.top_k(5, dim=1)
		# label_set = []
		# print('label_set_index',label_set_index)
		for labeli in range(0, batch_size):
			tempfalg = False
			addt = relabel[label_set_index[labeli]]
			if relabel[labeli] in relabel[label_set_index[labeli]]:
				tempfalg = True
			strpath = str(relabel[labeli]) + 'class' + str(relabel[label_set_index[labeli]]) + '_' + str(tempfalg) + '\n'
			if relabel[labeli] !=addt[0]:
				print(strpath)
			# f.write(strpath)
			# label_set.append(strpath)
		# print(label_set)
		

		# print('down!!')
		eyes_ = ops.eye(batch_size, batch_size, mindspore.int32)#diag is 1
		y_=relabel.expand(batch_size, batch_size).eq(relabel.expand(batch_size, batch_size).t()).float()
		score = ops.masked_select(score, (1-eyes_).bool()).view(batch_size,-1)
		y_ = ops.masked_select(y_, (1-eyes_).bool()).view(batch_size,-1)
		...
		group_y_=[]
		pointer=0
		# print('n_pos',n_pos) #tensor([6, 6, 6, 6, 6, 2])
		for i in range(len(n_pos)):
			...
			if n_pos[i]!=1:
				...
				pointer+=int(n_pos[i])
				# print('len of group x',group_x[cni])
				# print('len of group y',group_y_[cni])
				# print('i',i)

				_,index = group_x[cni].topk(self.k, dim=1)
				# print('index', index)
				for j in range(index.size(0)):
					predict_topk = group_y_[cni][j][index[j]]
					# print('predict_topk',predict_topk)
					real_gt_num = min(self.k , n_pos[i]-1)
					# print('real_gt_num',real_gt_num)
		...
		loss3=(mean-ops.eye(embd_size,embd_size, mindspore.int32)).norm()
		# print('loss3',loss3)
		# print('self.err_pos',self.err_pos)
		# print('self.sparsity', self.sparsity)
	...
	def N_posBiggerThanK(self,x, y_,k):
		...
		smooth_1, hard_1 = detect_large(x_1, k, self.tau, self.thresh)#fenbian hard easy with margin
