...
class LogSumExp(nn.Cell):
    def __init__(self, k, p=None, thresh=1e-5):
        ...
        self.register_buffer('grad_k', Tensor(0, mindspore.float32))
        self.register_buffer('grad_km1', Tensor(0, mindspore.float32))
...
class LogSumExp_F(nn.Cell):
    def __init__(self, k, p, thresh, mul, buffers):
        super(LogSumExp_F, self).__init__()
        self.k = k
        self.p = p
        self.mul = mul
        self.thresh = thresh
...
def log_sum_exp(x):
    ...
    """
    max_score, _ = x.max(1)
    return max_score + ops.log(np.sum(ops.exp(x - max_score[:, None]), 1))


def straight_forward_log_sum_exp(x,k):  
    combins = CNK(x.size()[1],k)
    new_x=ops.zeros(x.size()[0],len(combins)).cuda()
...
class LogSumExp_new(nn.Cell):
    def __init__(self, k):
        super(LogSumExp_new).__init__()
        self.k = k
    ...
    def construct(self,x):
        ...
        tbl=ops.ones(batch_size,n,self.k+1)
        tbl=tbl*(-100)
        ...
        for m in range(batch_size):
            ...
            for i in range(n-1):
                ...
                for k in range(self.k):
                    temp=ops.max(tbl[m][i][k],tbl[m][i][k+1]-x[m][i+1])
                    #print(temp)
                    tbl[m][i+1][k+1]=temp+ops.log(ops.exp(tbl[m][i][k]-temp)+ops.exp(tbl[m][i][k+1]-x[m][i+1]-temp))
    ...
    def backward(self,loss):
        ...
        tbl_gradient=ops.zeros(batch_size,self.k,n).cuda()
        for m in range(batch_size):
            for i in range(n):
                tbl_gradient[m][0][i]=1
                for j in range(self.k-1):
                    tbl_gradient[m][j+1][i]=ops.exp(tbl[m][n-1][j+1])-x[m][i]*tbl_gradient[m][j][i]
