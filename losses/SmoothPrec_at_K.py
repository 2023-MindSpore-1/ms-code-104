...
class SmoothPrec_at_K_Loss(nn.Cell):
    ...
    def construct(self, x, y):
        ...
        head=y[:,0]
        y_=ops.zeros(0,y.size(1)-1)
        for j in range(batch_size):
            categories=-1
            for i in range(y.size(1)):
                if y[j][i]==head[j]:
                    categories+=1
            temp_y=ops.concat([ops.ones(categories,mindspore.float32), ops.zeros(y.size(1)-1-categories)],0).reshape(1,y.size(1)-1)
            y_=ops.concat([y_,temp_y],0)
        y=y_
        n_pos=[]
        for i in range(batch_size):
            n_pos.append(int(y[i].sum()+1))
        score = ops.zeros(0,self.N+1)
        y_= ops.zeros(0,self.N)
        mask = ops.zeros(0,self.N)
        for i in range(batch_size):
            for j in range(n_pos[i]):      
                temp=ops.matmul(x[j+i*(self.N+1),:].view(-1,embd_size),x[0+i*(self.N+1):self.N+1+i*(self.N+1),:].t())/ops.sqrt((x[j+i*(self.N+1),:].view(-1,embd_size)**2).sum())/(x[i*(self.N+1):i*(self.N+1)+1+self.N,:]**2).sum(1)
                score=ops.concat([score,temp],0)
                mask=ops.concat([mask,self.get_1d_array(self.N,j).view(-1,self.N)],0)
                y_=ops.concat([y_,y[i].view(-1,self.N)],0)

        score = score.gather(1, mask.cuda().long())
        score_hat = score.add(self.margin*(1-y_))

        _,index = score.topk(self.k, dim=1)
        err_pos=2*ops.gt(index,self.n_pos-1).sum().float()/(score.size(0)*(score.size(1)))
    ...
    def get_1d_array(self,N,i):
        mask = ops.zeros(N,requires_grad=False)
        j = 0
        counter = 0
        while counter!=N:
            if j==i:
